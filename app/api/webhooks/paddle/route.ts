...
export async function POST(req: Request) {
  ...
  if (eventInsertError) {
    // Duplicate event is expected on retries; treat as success.
    if ((eventInsertError as { code?: string }).code === "23505") {
      return NextResponse.json({ success: true, received: true, duplicate: true });
    }
    console.error("[PADDLE_WEBHOOK_EVENT_INSERT_ERROR]", eventInsertError);
    return NextResponse.json({ success: false, error: "Failed to persist webhook event" }, { status: 500 });
  }

  // 2) Lightweight subscription snapshot (safe additive logic)
  // We support both transaction.* payload and subscription.* payload.
  ...
  // 2a) Customer mapping snapshot (idempotent by provider+customer)
  if (customerId) {
    const mappingRow = {
      provider: "paddle",
    ...
    if (mapErr) {
      console.error("[PADDLE_WEBHOOK_CUSTOMER_MAP_UPSERT_ERROR]", mapErr);
      // Do not fail webhook if mapping failed.
    }
  }

  // 2b) Subscription snapshot
  ...
  if (subscriptionId) {
    ...
    const { error: upsertError } = await admin
      ...
      .upsert(snapshot, { onConflict: "provider,provider_subscription_id" });

    if (upsertError) {
      console.error("[PADDLE_WEBHOOK_SUBSCRIPTION_UPSERT_ERROR]", upsertError);
      // Do not fail webhook delivery after event persisted.
    }
  }
